...
`ru-tcp.txt` (для белых списков), `awg.conf` и QR-коды `*.png` ко всему.
...
`vpn-health.timer` каждую минуту проверяет: REALITY на 443 отвечает как сайт, 8443 отдаёт сертификат
icloud, awg0 поднят, nginx жив, есть внешний интернет, сертификат не истекает, диск не полон,
RU-нода отвечает. Упавшее перезапускает, в Telegram пишет один раз при падении и один раз при
восстановлении. Чтобы бот привязался, напиши ему что угодно после установки.
