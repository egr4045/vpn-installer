...
`/users` показывает людей кнопками; нажатие на имя присылает готовую карточку для пересылки: фото с
небольшим QR, ссылки «скачать Amnezia», три шага, и файл `.vpn` с ключом, спрятанным под спойлер.
Кнопки под карточкой дают варианты: AmneziaWG, вход через Россию, запасной выход, подписка Happ,
удаление. Ключи `vpn://` собирает `bin/amnezia-key.py` в нативном формате клиента, по одному
протоколу на ключ, чтобы QR оставался читаемым.

| Команда | Что делает |
|---|---|
| `/users` | люди кнопками, у каждого статус AmneziaWG |
| `/add Имя` | новый человек на всех нодах и сразу его карточка |
| `/status` | вывод `vpn-status.sh` |
| `/newip` | новый IPv4 основной ноды с перевыпуском всего |
| `/qr Имя [ru\|nl\|awg\|x2\|happ\|clash]` | карточка без кнопок, если нужно |
