...
20 секунд), весь трафик идёт в него, иначе автоматически в запасной. Подписка Happ содержит оба
выхода, ключ `amnezia2` даёт запасной выход в AmneziaVPN.

`/newip` в боте (или `rotate-ip.sh`) берёт у Timeweb новый IPv4 для основной ноды, переписывает
...
Меню редактируют сами себя, чат не засоряется: `/users` показывает людей кнопками, тап по имени
превращает это же сообщение в панель человека (варианты ключа, ClashMi, Happ, удалить, назад), тап по
варианту присылает карточку отдельным сообщением. Карточка предназначена для пересылки, поэтому на ней
нет админ-кнопок: фото с QR и тремя шагами, URL-кнопки «iPhone / Android / Windows-Mac», затем файл
`.vpn` с ключом под спойлером (тап раскрывает, тап копирует). Ключи `vpn://` собирает
`bin/amnezia-key.py` в нативном формате клиента, по одному протоколу на ключ, чтобы QR оставался
читаемым.

| Команда | Что делает |
|---|---|
| `/users` | люди кнопками, 🟢 = в сети сейчас (xray или AmneziaWG) |
| `/add Имя` | новый человек на всех нодах и сразу его карточка |
| `/del Имя` | удалить человека, с подтверждением |
| `/status` | ноды, сайты, сертификат, диск, баланс из `health.json` — мгновенно; кнопка «Проверить сейчас» |
| `/newip` | новый IPv4 Амстердама с перевыпуском всего, с подтверждением |
| `/dropip 1.2.3.4` | отпустить лишний IP (сервер перезагрузится), с подтверждением |
| `/qr Имя [ru\|nl\|awg\|x2\|happ\|clash]` | карточка напрямую |

Прогресс («⏳ Создаю…») редактируется в результат, подтверждения после нажатия теряют кнопки, не-админ
по кнопке получает всплывашку, а не сообщение в чат. Словарь общий с алертами: 🇳🇱 Амстердам,
🇷🇺 Москва, 🛟 Запасной.

По умолчанию людям выдаётся ключ через RU-вход: он переживает белые списки, а падение выхода
компенсирует балансировщик на самой ноде. Кроме ключей Amnezia есть две подписки на человека:
`/s/<token>` для Happ (все xray-серверы, авто-выбор) и `/c/<token>` для ClashMi и других клиентов на
ядре mihomo (`bin/clash-sub.py`): там же AmneziaWG 3.1, группа «Авто» с приоритетом RU → Амстердам →
запасной, российские сайты напрямую, DNS over HTTPS.
...
чаще раза в час), диск (80 и 95 %), память, Москва и запасной отвечают, баланс Timeweb (раз в час, если
есть `TW_API_TOKEN`), перезагрузка сервера. Упавшее перезапускает и пробует снова; если ожило — молчит
(три самовосстановления за час дают одно 🟠). Алерт уходит после двух неудачных прогонов подряд,
восстановление приходит с длительностью простоя.
...
🟠 деградация — со звуком; 🟡 предупреждение заранее, 🟢 восстановилось, 🔵 инфо — тихо. Каждый день в
09:00 MSK приходит ☀️ дайджест: инциденты за сутки, сайты, сертификат, диск, баланс, кто был в сети;
не пришёл дайджест — Амстердам лежит. Если Telegram недоступен (нет интернета, перезагрузка), сообщения
ждут в спуле и уходят потом с пометкой о задержке.
...
из него бот рисует `/status`; журнал инцидентов — `incidents.log` рядом.
...
(🔴 и подсказка `/newip`), Амстердам недоступен целиком (🔴), заблокирован только :443 по SNI (🟠). Свой
