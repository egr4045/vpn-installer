...
inject_users() { # inject_users <rendered.json> <out.json> — fill both inbounds from users.json (+relay on exit node)
...
echo "rendered exit node: $n_users users (+relay), $n_peers awg peers"
...
if [ -n "${RU_HOST:-}" ]; then
  ...
  SSH="ssh -o BatchMode=yes -o ConnectTimeout=15 -o StrictHostKeyChecking=accept-new root@$RU_HOST"
  if scp -o BatchMode=yes -o ConnectTimeout=15 -o StrictHostKeyChecking=accept-new -q "$t2" "root@$RU_HOST:/usr/local/etc/xray/config.json" \
     && $SSH "chmod 644 /usr/local/etc/xray/config.json && systemctl restart xray && systemctl is-active xray" >/dev/null; then
