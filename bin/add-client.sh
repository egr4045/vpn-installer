...
# an AmneziaWG peer + a personal subscription (one QR for every xray server, for Happ-like clients).
...
if ! jq -e --arg n "$NAME" '.[]|select(.name==$n)|.sub' "$ETC/users.json" >/dev/null 2>&1 || [ "$(jq -r --arg n "$NAME" '.[]|select(.name==$n)|.sub // empty' "$ETC/users.json")" = "" ]; then
